...
- 成绩详情新增 Rating 与 Over Power 计算器
- 适配中二节奏 2027 的称号变化：
  - 修复佩戴单曲 ALL JUSTICE 称号时同步后玩家信息未更新的问题
  - 中二节奏称号 `color` 字段新增 `ultima` 取值

## 2026/9/11

- 修复通过 HTTP 代理上传成绩时进度一直显示“排队中”、直到上传完成才更新的问题
- 修复切换曲目后再次查看谱面详情时物量可能不显示的问题
- 同步游戏数据：
  - 页面头部新增「同步教程」菜单，支持快捷查看完整图文、视频教程及常见问题（[#101](https://github.com/Lxns-Network/maimai-prober-frontend/pull/101)）
  - 代理配置与微信打开步骤中增加各系统详细图文与传分注意事项链接

## 2026/9/10

- 支持中二节奏 2027：
  - 成绩与 Rating 历史页面现在可以选择最新版本
  - 公开 API 的 `version` 查询参数默认值更新为 `24000`

## 2026/9/8

- 游玩历史：
  - 成绩详情现在按上传时间显示全部成绩更新，修复缺少游玩时间的更新被遗漏、最高成绩可能不在趋势中的问题
  - 舞萌 DX、中二节奏成绩历史接口现在按 `upload_time` 返回全部成绩更新，不再仅返回含 `play_time` 的记录
...
- 修复登录会话过期重定向后未弹出提示弹窗的问题

## 2026/8/22

- 上传成绩时现在会忽略客户端传入的游玩时间（`play_time`），该时间仅可通过 NET 的 HTML 源代码上传写入
- OAuth 授权时若使用的 `client_id` 已失效，现在会返回 404，客户端可以据此判断并重新注册应用
...
- 新增通知中心
- 成绩管理页面的分数构成新增定数分析，反推各达成率档所需的最低定数
- 开发者面板新增 API 用量统计
- 创建曲目别名时支持实时查重
- 现在可使用返回键关闭弹窗
- 曲目搜索现在会高亮匹配的内容
...
- 优化了舞萌 DX、中二节奏 Rating 趋势图的加载速度
- 优化了成绩排行榜的加载速度

## 2026/6/11

- 重新设计成绩管理页面，更好地适配桌面端与移动端：
  - 排序与筛选整合至列表顶部工具栏，排序改为下拉菜单
  - 桌面端筛选面板常驻页面右侧，筛选实时生效；移动端筛选改为底部抽屉
  - 成绩列表翻页后自动回到列表顶部
...
- 修复舞萌 DX 新成绩的 DX 星级没有被计算的问题
- 修复中二节奏 Rating 趋势图显示不正确的问题
- 修复中二节奏在开启允许覆盖最佳成绩后，部分成绩会被覆盖的问题
...
- 提升了同步游戏数据时的稳定性
- 同步游戏数据页面现在可以跳过配置 HTTP 代理步骤，直接监听同步进度
