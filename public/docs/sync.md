...
如果网络环境受限无法配置 HTTP 代理，也可以在浏览器登录 NET 页面后，将完整的网页源代码保存为 `.html` 或 `.txt` 文件（或直接复制 HTML 文本），在[同步游戏数据页](/user/sync)的「上传 HTML」中进行导入。

目前支持玩家信息、收藏品、最近游玩记录、最佳成绩等页面源码。
...
::: info 节点显示超时？
在 Clash 或 Shadowrocket 客户端中，节点测速显示“超时”属于正常现象。查分器代理节点仅用于处理游戏同步相关流量，不响应通用的延迟测试请求，不影响正常同步，启用系统代理后继续执行后续步骤即可。
:::
