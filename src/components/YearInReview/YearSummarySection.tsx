...
const CardImage = ({ game, image, title, description, percentage }: CardImageProps) => {
  return (
    ...
    >
      ...
      <Text className={classes.description}>占查分器全年上传谱面的 {percentage}%</Text>
    </Card>
  );
};
...
export const YearSummarySection = ({ data }: { data: YearInReviewProps }) => {
  const playerTotal = Object.values(data.player_total_uploads).reduce(
    (sum, value) => sum + value,
    0,
  );
    ...
    <Grid>
      <Grid.Col span={{ base: 12, sm: 6 }}>
        <Card className={classes.card} shadow="md" withBorder radius="md">
          <div>
            <Text className={classes.description}>上传谱面统计</Text>
            <Title order={3} className={classes.title}>
              看看你今年上传了多少次谱面吧！
            </Title>
          </div>
          {filteredChartData.length > 0 ? (
            <DonutChart labelsType="percent" withLabels mx="auto" data={filteredChartData} />
          ) : (
      ...
      <Grid.Col span={{ base: 12, sm: 6 }}>
        <Card className={classes.card} shadow="md" withBorder radius="md">
          <div>
            <Text className={classes.description}>全年上传天数统计</Text>
            <Title order={3} className={classes.title}>
              今年你有记录的上传天数达到了 {data.player_upload_days} 天，占全年{" "}
              {dayInYearPercentage}%！
              {(() => {
          ...
          <Text className={classes.description}>这一年，你打卡了多少天？</Text>
        </Card>
      </Grid.Col>
    </Grid>
  );
