...
export const RatingGrowthSection = ({ data }: { data: YearInReviewProps }) => {
  ...
  const { growth, topSongs } = useMemo(() => {
      ...
      if ("bests" in bests) {
        ...
        if (data.year === 2025) {
          if (bests.new_bests.length === 0) {
            return (
              Math.floor(((b30Sum + bests.bests[0].rating * 10) / 40) * 100) / 100 || 0
            ).toFixed(2);
          }
        }
    ...
    const getTopSongs = (bests: MaimaiBestsProps | ChunithmBestsProps) => {
      ...
      if ("bests" in bests) {
        return bests.bests.sort((a, b) => (b.rating || 0) - (a.rating || 0)).slice(0, 6);
      }
      ...
      <Stack gap="lg">
        <div>
          ...
          <Title
            order={3}
            fw={900}
            style={{ fontSize: "32px", marginTop: "var(--mantine-spacing-xs)" }}
          >
            {data.year} 年，你的 Rating {growth.isPositive ? "提升" : "下降"}了{" "}
            {data.game === "maimai"
              ? Math.abs(growth.diff).toFixed(0)
              : Math.abs(growth.diff).toFixed(2)}
            ！
          </Title>
          {data.game === "chunithm" && (
        ...
        <Group justify="space-between">
          ...
          <Badge
            size="xl"
            leftSection={
              growth.isPositive ? <IconTrendingUp size={18} /> : <IconTrendingDown size={18} />
            }
