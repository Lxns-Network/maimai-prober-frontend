        ...
        {keys(basic).map((type) => {
          if (key === "break" && type === "perfect") {
              ...
              return (
                <Table.Td key={type}>
                  <Text size="sm" c="yellow">
                    {critical_perfect_bonus.toFixed(4)}%
                  </Text>
                  <div>{(break_bonus.perfect[0] / notes.break).toFixed(4)}%</div>
                  <div>
                    {(critical_perfect_bonus - break_bonus.perfect[1] / notes.break).toFixed(4)}%
                  </div>
                </Table.Td>
              );
            }
              ...
              return (
                <Table.Td key={type}>
                  ...
                  {break_bonus.perfect.map((v) => {
                    return (
                      <div key={v}>{(v / notes.break - critical_perfect_bonus).toFixed(4)}%</div>
                    );
                  })}
                </Table.Td>
              );
            ...
            return (
              <Table.Td key={type}>
                <Text size="sm" c="yellow">
                  {(percentage + critical_perfect_bonus).toFixed(4)}%
                </Text>
                {break_bonus.perfect.map((v) => {
      ...
      <Table mb={-12} horizontalSpacing={0} layout="fixed">
        <Table.Thead>
          <Table.Tr>
            ...
            <Table.Th w={10} c="orange">
              PERFECT
            </Table.Th>
            <Table.Th w={10} c="pink">
              GREAT
            </Table.Th>
            <Table.Th w={10} c="green">
              GOOD
            </Table.Th>
            <Table.Th w={10} c="gray">
              MISS
            </Table.Th>
          </Table.Tr>
        </Table.Thead>
