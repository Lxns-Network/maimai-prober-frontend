...
export const ScoreModalMenu = ({ score, difficulty, onClose }: ScoreModalActionMenuProps) => {
  ...
  const deletePlayerScoresHandler = () => {
    deleteScoreHistoryMutation.mutate(
      { game, params },
      {
        onSuccess: () => {
          notifications.show({
      ...
      <Menu.Dropdown>
        ...
        {"type" in score && (
          <>
            {difficulty && "is_buddy" in difficulty && difficulty.is_buddy ? (
              <Menu.Sub>
                <Menu.Sub.Target>
                  <Menu.Sub.Item leftSection={<IconPlayerPlay size={20} stroke={1.5} />}>
                    谱面预览
                  </Menu.Sub.Item>
                </Menu.Sub.Target>
              ...
              <Menu.Item
                leftSection={<IconPlayerPlay size={20} stroke={1.5} />}
                onClick={() => {
        ...
        {score.upload_time && (
          <>
            ...
            <Menu.Item
              color="red"
              leftSection={<IconClearAll size={20} stroke={1.5} />}
              onClick={() => {
                openConfirmModal(
                  "删除成绩",
                  "你确定要删除所有历史成绩吗？",
