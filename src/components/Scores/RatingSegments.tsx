  ...
  if (isMaimaiBests(bests)) {
    if (bests.standard) {
      data.push({
        ...
        avg:
          Math.floor(
            (bests.standard.reduce((acc, score) => acc + score.dx_rating, 0) /
              bests.standard.length) *
              100,
          ) / 100 || 0,
    ...
    if (bests.dx) {
      data.push({
        ...
        avg:
          Math.floor(
            (bests.dx.reduce((acc, score) => acc + score.dx_rating, 0) / bests.dx.length) * 100,
          ) / 100 || 0,
    ...
    if (bests.bests) {
      data.push({
        ...
        count:
          Math.floor(
            (bests.bests.reduce((acc, score) => acc + TruncateToTwoDecimal(score.rating), 0) /
              bests.bests.length) *
              100,
          ) / 100 || 0,
        color: "#228be6",
    ...
    if (bests.selections) {
      data.push({
        ...
        count:
          Math.floor(
            (bests.selections.reduce((acc, score) => acc + TruncateToTwoDecimal(score.rating), 0) /
              bests.selections.length) *
              100,
          ) / 100 || 0,
        color: "#228be6",
    ...
    if (bests.new_bests) {
      data.push({
        ...
        count:
          Math.floor(
            (bests.new_bests.reduce((acc, score) => acc + TruncateToTwoDecimal(score.rating), 0) /
              bests.new_bests.length) *
              100,
          ) / 100 || 0,
        color: "#228be6",
  ...
  return (
    <Paper withBorder p="md" radius="md">
      <Stack align="center" gap={4}>
        <Text fw={500}>{game === "maimai" ? "DX 评分总和" : "Rating 均值"}</Text>
        ...
        {"bests" in bests && "new_bests" in bests && data.length === 3 && (
          <RatingBadge
            ...
            rating={
              Math.floor(
                ((data[0].count * (bests.bests?.length || 0) +
                  data[2].count * (bests.new_bests?.length || 0)) /
                  50) *
                  100,
              ) / 100
            }
