...
const ScoreHistoryChart = ({ scores }: { scores: ChunithmScoreProps[] }) => {
  let ticks = [
    800000, 900000, 925000, 950000, 975000, 990000, 1000000, 1005000, 1007500, 1009000, 1010000,
  ];
  let min = 1010000;
      ...
      <AreaChart data={scores}>
        ...
        <YAxis
          ...
          domain={([dataMin, dataMax]) => {
            return [
              Math.max(Math.floor(dataMin) - 10000, 800000),
              Math.min(Math.floor(dataMax) + 10000, 1010000),
            ];
          }}
          ticks={ticks}
          fontSize={14}
...
export const ChunithmScoreHistory = ({
  scores,
  minScore,
}: {
  ...
  return (
    <>
      ...
      {scores.length < scoresLength && (
        <Text fz="xs" c="dimmed">
          ※ 为了方便观察分数变化，图表过滤了 {scoresLength - scores.length} 条分数低于{" "}
          <NumberFormatter value={minScore} thousandSeparator /> 的成绩。
        </Text>
      )}
    </>
  );
