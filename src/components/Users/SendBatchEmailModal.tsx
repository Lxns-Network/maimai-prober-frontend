...
export const SendBatchEmailModal = ({
  users,
  opened,
  close,
}: {
    ...
    extensions: [
      StarterKit.configure({
        ...
        code: {
          HTMLAttributes: {
            style:
              "background-color: #F8F9FA; padding: 1px 5px; font-size: 12px; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, Liberation Mono, Courier New, monospace;",
          },
        },
        horizontalRule: {
          HTMLAttributes: {
    ...
    >
      <form
        onSubmit={form.onSubmit((values) => {
          openConfirmModal("确认发送", `确认要向 ${users.length} 名用户发送邮件吗？`, () =>
            sendBatchEmailHandler(values),
          );
        })}
      >
        <TextInput
