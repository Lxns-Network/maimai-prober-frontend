...
export const WorkersSection = () => {
  ...
  const handleDelete = (worker: WorkerProps) => {
    openConfirmModal(
      "删除工作节点",
      `确定要删除工作节点「${worker.name}」吗？删除后将断开连接并移除记录。`,
      async () => {
        try {
  ...
  const handleBatchDelete = () => {
    openConfirmModal(
      "批量删除工作节点",
      `确定要删除所选的 ${selectedWorkers.length} 个工作节点吗？`,
      async () => {
        try {
        ...
        <DataTable
          ...
          columns={[
            ...
            },
            {
              ...
              title: "创建时间",
              width: 150,
              render: ({ id, create_time }) =>
                id === 0 ? "-" : new Date(create_time).toLocaleString(),
              sortable: true,
            },
            {
              accessor: "actions",
          ...
          rowExpansion={{
            ...
            expanded: {
              ...
              onRecordIdsChange: (ids: unknown[]) =>
                setExpandedWorkerIds((ids as number[]).filter((id) => id !== 0)),
            },
            content: ({ record }) => (
              <Box p="md">
                <Grid>
                  ...
                  <Grid.Col span={6}>
                    <TextInputWithCopyButton
                      label="远程地址"
                      value={record.online ? record.remote_addr || "-" : "-"}
                    />
                  </Grid.Col>
                </Grid>
              </Box>
          ...
          allRecordsSelectionCheckboxProps={{
            indeterminate:
              ...
              selectedWorkers.length < sortedWorkers.filter((w) => w.id !== 0).length,
            checked:
              sortedWorkers.filter((w) => w.id !== 0).length > 0 &&
              selectedWorkers.length === sortedWorkers.filter((w) => w.id !== 0).length,
            onChange: () => {
