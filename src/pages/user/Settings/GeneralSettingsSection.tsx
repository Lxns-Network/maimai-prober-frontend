  ...
  maimai: [
    ...
    },
    {
      ...
      description: (
        <>
          设置每次爬取时使用的爬取方式。
          <Anchor component={Link} to="/docs/settings#爬取谱面成绩的方式">
            了解更多
          </Anchor>
        </>
      ),
  ...
  const updateUserConfigHandler = (key: string, value: unknown) => {
    if (key === "allow_overwrite_best_score" && value === true) {
      openConfirmModal(
        "开启「允许覆盖最佳成绩」",
        <>
          若没有<Mark>必须覆盖错误成绩</Mark>的明确需求，建议保持关闭，以避免因爬取异常导致的{" "}
          <Mark>B50 不准确</Mark>、<Mark>部分成绩被错误覆盖</Mark>等问题。开启后，建议在爬取前先
          <Anchor size="sm" component={Link} to="/user/scores?tab=backup">
            备份成绩
          </Anchor>
          ，以防万一。
        </>,
    ...
    <div>
      <Card withBorder radius="md" className={classes.card} mb="md">
        ...
        <SettingList
          ...
          data={(crawlConfigData as never)[game ? game : "maimai"]}
        />
      </Card>
      <Card withBorder radius="md" className={classes.card} mb="md">
        <LoadingOverlay visible={isLoading} overlayProps={{ radius: "sm", blur: 2 }} zIndex={1} />
        ...
        <Text fz="xs" c="dimmed" mt={3} mb="lg">
          将影响第三方开发者通过查分器 API 访问你的「{game === "chunithm" ? "中二节奏" : "舞萌 DX"}
          」数据
        </Text>
        <SettingList
          onChange={updateUserConfigHandler}
          ...
          data={[
            {
              ...
              onClick: () =>
                openConfirmModal(
                  "解绑游戏账号",
                  <>
                    你确定要解绑你的「{game === "chunithm" ? "中二节奏" : "舞萌 DX"}
                    」账号吗？解绑后，你可以随时重新同步游戏数据，或切换其他查分器账号绑定。
                  </>,
                  unbindPlayerHandler,
                ),
            },
              ...
              onClick: () =>
                openConfirmModal(
                  ...
                  <>
                    你确定要
                    <Mark>
                      删除你的查分器账号里所有的「{game === "chunithm" ? "中二节奏" : "舞萌 DX"}
                      」谱面成绩
                    </Mark>
                    吗？这将包括<Mark>所有历史爬取的谱面成绩</Mark>，并且不可撤销。
                  </>,
