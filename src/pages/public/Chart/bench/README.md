...
| 真实播放 `--playback`      | **无可测差异**：掉帧率 10.3% → 10.5%，interval p95 26.1ms、p99 27.5ms 前后一致（各 3 次交替）             |
| 像素回归                   | 通过：13 帧最大通道差 43/255，超 8/255 的像素每帧 ≤ 9，MAE ≤ 0.04/255                                     |
| 处置                       | **保留**                                                                                                  |

> **吞吐变快 ≠ 少掉帧。** 压测谱面在本机远超帧预算：掉帧处的 rAF 间隔是 ~26ms，而刷新周期只有 13.3ms，也就是 GPU 稳定地要用掉两个刷新周期。11% 的吞吐提升不足以把它拉回一个周期内，所以掉帧率纹丝不动；而真实谱面本来就是 0 掉帧，同样没有可改善的余地。**这条改动的正确说法是"同样画面少用约 11% 的 GPU 填充"，不是"修好了卡顿"。**
...
touch 花瓣改紧尺寸分层精灵（`TouchRenderer`，224² → 160²）——**目前最大的一块未取用收益**：

| 项目     | 结果                                                                                                        |
| -------- | ----------------------------------------------------------------------------------------------------------- |
| 填充收益 | **每帧合成面积 33.81 → 19.53 Mpx（−42%）**，overdraw 1352% → 781%。确定性测量，不受 GPU 争抢影响            |
| 墨迹安全 | 安全：224² 精灵实测最小透明边距 37px、墨迹跨度仅 146px，裁到 160² 不会切到任何墨迹（含投影）                |
| 像素回归 | **不通过 strict**：约 0.7% 像素变化，最大通道差 ~50/255，集中在花瓣抗锯齿边缘，无结构性改变、无成片单向变暗 |
| 处置     | **在"视觉零偏差"约束下不取用**，patch 存 `.bench/touch-petal-sprite-crop.patch`                             |

> 试过把裁剪余量从 1 加到 8 / 16 / 24 想换取逐像素一致：**没用**。余量 24 时面积只降到 30.77 Mpx（−9%），最大通道差仍有 41。说明偏差不是"裁到墨迹"或"边缘采样"造成的（那样加余量就该收敛），而是**重新按不同尺寸烘焙后，Chromium 的重采样相位本身就变了**。当前 dest 尺寸相对 source 有约 0.999 的非 1:1 缩放，这个缩放让"换个 source 尺寸"必然改变输出。
>
> 也就是说：**这 42% 想拿，就必须接受一点点重采样差异**（人眼不可见，但不是 0）。要么接受，要么先把精灵改成严格 1:1 映射——而那本身也会改变当前画面。
...
**任何"性能优化"在合并前都必须先用配对基准证明它真的减少了工作量，再用 `--prod --playback` 说明它属于哪一类：**

- **改善帧交付**：掉帧数或 interval p99 变好——这才是用户能感知的。
- **只减少工作量**：吞吐/填充量下降但掉帧不变（本次的 sprite 收紧就是这类）。仍然值得做（省 GPU、省电、在更弱的机器上留余量），但**不能宣称修好了卡顿**。
- **什么都没测出来**：配对置信区间跨过 0，就当它不存在，撤掉。

三类都要如实写进 PR，不要用第一类的说法去描述第二类。 前面列过的 canvas 侧候选项（星星头精灵、波纹环精灵、clip 改盖角、分配清理）在本机数据下全部没有可测收益，暂不实施。
