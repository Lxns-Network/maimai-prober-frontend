...
真实谱面（系ぎて [DX] Re:MASTER，1366 notes，同样 13 个时刻）则完全是另一幅样子：

| 精灵源     | 每帧贴图数 | 每帧面积 | 占比 |
| ---------- | ---------- | -------- | ---- |
| 1581² 图层 | 1          | 1.35 Mpx | 87%  |
| 590² 烟花  | 6          | 0.14 Mpx | 9%   |
| 480² tap   | 2          | 0.05 Mpx | 3%   |

**每帧只合成 1.54 Mpx = 画布的 62%，连一次 overdraw 都没有，touch 花瓣一次都没出现。**比压测谱面少 22 倍。

这条对比很重要，别搞反了：

- 压测谱面每 1/16 拍固定放 4 颗 touch，是**人为造出来的填充瓶颈**，用来放大差异、让改动可测。
- 真实谱面上根本不存在填充压力，所以**任何填充优化在真实内容上都约等于 0**——这也正好解释了为什么 sprite 收紧在压测谱面上有 +11%，`--playback` 的掉帧数却纹丝不动，以及为什么真实谱面本来就是 0 掉帧。
- 因此"要不要为了 −67% 的填充去接受一点重采样差异"这个取舍，**在真实内容上并不值得**：省下来的是一个用户碰不到的瓶颈。

定位热点前先用 `yarn chart:fill --chart <id>` 确认目标谱面的真实构成，不要拿压测谱面的分布去指导优化取舍。
...
试过两种独立实现，各扫了四档余量，结论一致：**只要减少 touch 花瓣的合成面积，像素就会变。**

| 实现                                          | 每帧面积              | 最大通道差 | 余量扫描                 |
| --------------------------------------------- | --------------------- | ---------- | ------------------------ |
| 重新按小尺寸烘焙（`touch-petal-sprite-crop`） | 19.53 Mpx（−42%）     | ~50        | 1 / 8 / 16 / 24 均不收敛 |
| 源矩形裁剪（`touch-petal-ink-rect`，更优）    | **10.98 Mpx（−67%）** | ~41        | 1 / 2 / 4 / 8 均不收敛   |

源矩形版按 alpha 自动求每张精灵的墨迹矩形，只贴 ink 子矩形，并把目标矩形按**同一仿射映射**平移（`dest = origin + inkOffset × scale`），数学上和整张贴图是同一个映射；每层各自求包围盒，所以比统一裁到 160² 还省。

即便如此仍有约 0.44% 像素、最大 41/255 的差异，且**加余量完全不收敛**——排除了"裁到墨迹"和"边缘采样"两种解释。根因是**当前 blit 本身不是 1:1**：精灵按 `ceil(half*2*backingScale)` 烘焙，而目标尺寸是未取整的 `half*2*backingScale`，两者差约 0.999 倍。既然每次贴图都在重采样，换一个 source 区域就会换一套采样，输出必然变。

**真正的解锁条件是把这次 blit 改成严格 1:1**（源尺寸 = 目标设备尺寸）。那样裁掉整数个全透明像素才是逐像素等价的——但"改成 1:1"本身就会改变当前画面，因此在"视觉不得与现状偏差"的前提下同样不能做。

所以这 67% 与零偏差互斥，不是实现没做到位。要拿就得接受约 0.44% 像素、最大 41/255 的重采样差异（集中在花瓣抗锯齿边缘，无结构性改变、无成片单向变暗）；真要取用请用 `.bench/touch-petal-ink-rect.patch`，它比早先那份 `touch-petal-sprite-crop.patch` 省得多、偏差也更小。
